...
class Program
{
    ...
    {
    Sezar cipher = new Sezar(3);
    string plainText = "Hello World!";
    string encrypted = cipher.Encrypt(plainText);
    Console.WriteLine($"Encrypted: {encrypted}");
    string decrypted = cipher.Decrypt(encrypted);
    Console.WriteLine($"Decrypted: {decrypted}");
    string cipherInfo = cipher;
    Console.WriteLine(cipherInfo);
    string secretMessage = "Khoor Zruog!";
    Sezar decoder = (Sezar)secretMessage;
    string revealed = decoder.Decrypt(secretMessage);
    Console.WriteLine($"Revealed: {revealed}");
    }
}